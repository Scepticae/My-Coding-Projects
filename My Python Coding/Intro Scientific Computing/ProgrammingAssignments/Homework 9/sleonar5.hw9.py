...
#  
# This program uses romberg integration to approximate the integral for the 
# function in problem 6.1.14 from 0 to 1/u. The romberg method uses the trapezoidal 
# rule in conjunction with richardson extrapolation to evaluate the integral(I).
...
# Defines the function from problem 6.1.14 for Heat Capacity
def f(x):
  if x == 0: return 0
  else: return (x**4 * exp(x)) / ((exp(x) - 1)**2)   # define the integrable function here


# Creates an array of values from 0 to 1 in increments of 0.5
u = arange(0,1.01,0.05)

# Prints the Table Heading
print ("    u\t   g(u)")

#list that will contain all of g(u)s (y-coordinates for your plot)
gu = []


# Iterates through all values of 'u' and evaluates the integral for function f(x)
# to solve for g(u)
for i in u:
  if i == 0: g = 0.0;
  else:
	...
	# Multiply the integral by the u^3 component
    g = I * i**3            # evaluate g(i) here
  print ('{:6.2f}{:13.6f}'.format(i,g))
  gu.append(g) # Keeps values for plot
